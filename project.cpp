...
public:
          ...
          while (loginAttempt < 5)
    {
        ...
        if (compareIgnoreCase(userName, "Archit") && compareIgnoreCase(userPassword, "12345"))
        {
            cout << "Welcome Archit!\n";
            cout << "You are now logged in!\n\n";
            return true;
        }
        else if (compareIgnoreCase(userName, "Harsh") && compareIgnoreCase(userPassword, "12345"))
        {
            cout << "Welcome Harsh!\n";
            cout << "You are now logged in!\n\n";
            return true;
...
struct Student {
    ...
    string remarks; // Add a field for remarks
    Student* next;
};
...
public:
  ...
  void addStudent(const int& rollno, const string& firstName, const string& lastName, const string& course,
                ...
                bool feePaid, const string& remarks) {
    ...
    if (file.is_open()) {
        int rollno, year;
        string firstName, lastName, course, hostelName, fatherName, motherName, fatherMobile, studentMobile, feePaidStr, remarks;
        bool feePaid;

        while (file >> rollno >> firstName >> lastName >> course >> hostelName >> year >> fatherName >> motherName >> fatherMobile >> studentMobile >> feePaidStr >> remarks) {
            // Read the remarks from the CSV file
            getline(file, remarks, '\n');

            // Remove the leading ',' from the remarks
            if (!remarks.empty() && remarks[0] == ',') {
                remarks.erase(0, 1);
            }

            feePaid = (feePaidStr == "Yes");

            addStudent(rollno, firstName, lastName, course, hostelName, year, fatherName, motherName, fatherMobile, studentMobile, feePaid, remarks);
        }
    ...
    void listStudents() {
        ...
        while (current != nullptr) {
            ...
            cout << "Remark: "<<current->remarks <<endl;
            cout<<endl;
            current = current->next;
        }
    }
  ...
  void modifyStudent(int rollNo, const string& firstName, const string& lastName, const string& course,
   ...
   if (studentToUpdate != nullptr) {
       ...
       studentToUpdate->remarks = remarks;
...
int main() {
    ...
    if (auth.login_status()) {
        ...
        string firstName, lastName, course, hostelName, fatherName, motherName, fatherMobile, studentMobile,remarks;
        int year;
        bool feePaid;
            ...
            switch (choice) {
                case '0':
                    ...
                    if (result != nullptr) {
                        ...
                        cout<<"Remark: "<<result->remarks<<endl;
                    } else {
                ...
                case '1':
                    ...
                    getline(cin, remarks);
                    database.addStudent(rollno, firstName, lastName, course, hostelName, year, fatherName, motherName, fatherMobile, studentMobile, feePaid, remarks);
                    break;;
                ...
                case '3':
                   ...
                   cout << "Enter Remarks: ";
                    cin.ignore(); // Clear the input buffer
                    getline(cin, remarks);
                    database.modifyStudent(rollno, firstName, lastName, course, hostelName, year, fatherName, motherName, fatherMobile, studentMobile, feePaid, remarks);
                    break;
                case '4':
